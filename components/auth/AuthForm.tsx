...
type AuthFormType = 'login' | 'register' | 'reset' | 'new'
...
type AuthConfigBase = {
  ...
  input: typeof LIST_INPUTS_LOGIN
  schema: z.ZodTypeAny
  cta: string
  ctaLink: string
  ctaReset?: string
...
const CONFIG: Record<AuthFormType, AuthConfigBase> = {
  login: {
    title: 'Masuk',
    pageTitle: 'Masuk',
    onSubmit: signIn,
...
const AuthForm = ({ type }: { type: AuthFormType }) => {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      const message =
        type === 'reset'
          ? error.message
          : FIREBASE_ERRORS_CODE[error.code] || FIREBASE_ERRORS_CODE.default
  ...
  const formTitle = useMemo(() => CONFIG[type].pageTitle, [type])

  return (
    <form
      className="h-screen flex flex-col items-center pb-5 gap-10 relative z-20 bg-primary-dark"
      onSubmit={handleSubmit(onSubmit)}
    >
      <Link
        href={type === 'login' ? '/' : CONFIG[type].ctaLink}
        className="absolute left-2 top-2 z-10 w-9 h-9 flex items-center justify-center rounded-full border border-white/40 text-white"
        aria-label="Kembali"
      >
      ...
      <div className="relative flex flex-col justify-center items-center gap-1 px-5 w-full bg-primary-orange/80 h-1/5">
        <h1 className="text-xl font-bold text-white">{formTitle}</h1>
        <img src="/images/bg-auth.png" alt="bg-auth" className="absolute w-4/5 h-full py-2" />
      </div>
      ...
      <div className="flex flex-col gap-2 md:gap-5 w-4/5 md:w-1/2">
        {CONFIG[type].input.map((input) => (
          <div key={input.name} className="flex flex-col gap-y-1">
            ...
            <div className="rounded-2xl bg-cream border border-white/50 flex items-center gap-2 px-3 py-2">
              {input.icon}
              <input
                id={input.name}
                ...
                className="grow text-black bg-transparent outline-none placeholder:text-gray-500"
            ...
            {(errors as Record<string, any>)[input.name] && (
              <p className="text-xs text-red-600 mt-2 text-end">
                {(errors as Record<string, any>)[input.name]?.message}
              </p>
            )}
          </div>
        ))}
        {type === 'login' && (
          <Link className="self-end text-white" href={CONFIG[type].resetLink!}>
          ...
          <>
            <div className="flex items-center gap-2 mt-2 text-sm">
              <input
                ...
                {...register('terms')}
                disabled={isSubmitting}
                className="w-5 h-5 accent-primary-orange"
              />
              <label htmlFor="terms" className="text-white text-xs">
                Saya menyetujui aplikasi ini untuk mengumpulkan data sesuai dengan{' '}
                <Link href="/privacy-policy" className="text-primary-orange hover:text-primary-brass">
                  kebijakan privasi.
                </Link>
              </label>
            </div>
            {(errors as Record<string, any>).terms && (
              <p className="text-xs text-red-600 mt-2 text-start">
                {(errors as Record<string, any>).terms?.message}
              </p>
            )}
          </>
        )}
        ...
        <button
          className="w-3/4 mt-5 self-center text-cream bg-gray-black border border-cream rounded-2xl transition-colors duration-200 px-10 py-2 disabled:opacity-60"
          type="submit"
          disabled={isSubmitting || Boolean(successMessage) || isButtonDisable}
        >
        ...
        <Link className="self-center text-sm text-white mt-5" href={CONFIG[type].ctaLink}>
          {CONFIG[type].cta}
        </Link>
      </div>
    </form>
