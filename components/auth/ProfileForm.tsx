...
const ProfileForm = () => {
  ...
  const { user, isReady, loginUser } = useAuth()
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      await updateProfileUser(String(user.id), {
        ...data,
        birthdate: `${data.year}-${data.month}-${data.day}`,
        points: isEditing ? pointUser : pointUser + 80,
      })
      loginUser({
        user: {
          ...user,
  ...
  return (
    <form
      className="h-screen flex flex-col items-center gap-10 relative z-20 bg-primary-dark pb-5"
      ...
      <div className="flex flex-col gap-2 md:gap-5 w-4/5 md:w-1/2">
        {LIST_INPUTS.map((input) => (
          <div key={input.name} className="flex flex-col gap-y-1">
            {input.name === 'info' && !isEditing && (
              <label className={`${input.className} text-white`}>{input.label}</label>
            )}
            {input.name !== 'info' && input.label !== 'Birthdate' && (
              <label className={`${input.className} text-white`}>{input.label}</label>
            )}
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <select
                    ...
                    className="rounded-2xl bg-cream text-black px-2 py-2"
                  >
