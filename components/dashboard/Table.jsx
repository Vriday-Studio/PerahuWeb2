...
export const Table = ({ cols, data, loading, onPaginationChange, rowCount, pagination, handleDelete, canEditData, customEditData, deleteAction, userList, hideAction, hasPagination }) => {
    ...
    const columns = useMemo(
        () => [
                ...
                ? [
                      {
                          ...
                          cell: ({ row }) => (
                              <div className="flex gap-1">
                                {customEditData && customEditData(row.original)}
                                {canEditData && (
                                  <Link href={`${pathname}/${row.original.id || 0}`}>
                                  ...
                                  {userList && (
                                      <Link href={`${pathname}/${row.original.id}/users`}>
                                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="#888888" width="25" height="25">
            ...
            ...cols.map(({ id, header, cell }) => ({
                ...columnHelper.accessor(id, {
                    header,
                    cell: cell || (({ row }) => row.original[id] || "-"),
                }),
            })),
        ],
        [cols]
    ...
    return (
        <section className="w-full overflow-hidden bg-white shadow-lg rounded-lg">
            <table className="w-full table-fixed">
                <thead className="bg-gray-50">
                    {tableLib.getHeaderGroups().map((headerGroup) => (
                        <tr key={headerGroup.id}>
                            {headerGroup.headers.map((header) => (
                                <th key={header.id} className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider sm:px-4">{flexRender(header.column.columnDef.header, header.getContext())}</th>
                            ))}
                        </tr>
                    ))}
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                    {(data.length === 0 || rowCount === 0) && !loading && (
                        <tr>
                            <td colSpan="100%" className="px-6 py-4 text-center text-gray-500">No data</td>
                        </tr>
                    )}
                    {loading ? (
                        <tr>
                            <td colSpan="100%" className="px-3 py-4 text-center sm:px-4">
                                <Loading />
                            </td>
                        </tr>
                    ) : (
                        tableLib.getRowModel().rows.map((row, index) => (
                            <tr key={row.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                                {row.getVisibleCells().map((cell) => (
                                    <td key={cell.id} className="px-3 py-3 text-sm text-gray-900 whitespace-normal break-words sm:px-4">{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>
                                ))}
                            </tr>
                        ))
                    )}
            ...
            {pagination && hasPagination && <Pagination tableLib={tableLib} sizes={[5, 10, 20, 50, 100]}  rowCount={rowCount}/>}
        </section>
    );
};
