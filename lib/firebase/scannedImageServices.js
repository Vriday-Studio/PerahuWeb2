...
export const addImageScannedByUser = async (user, imageId) => {
    try {
        ...
        const todayServerScanCount = await get(ref(database, `TodayUsersPerahu/${formattedTime}/count/scan`));
        const todayServerLogScanRef = ref(database, `TodayUsersPerahu/${formattedTime}/scan/${todayServerScanCount.val() + 1}`);
        ...
        await set(ref(database, `TodayUsersPerahu/${formattedTime}/count/scan`), todayServerScanCount.val() + 1);
