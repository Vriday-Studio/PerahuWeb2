...
export const getPopularMerchandise = async () => {
    try {
        const redeemCode = await get(ref(database, `RedeemCode`));
        const redeemCodes = redeemCode.val();
        ...
        Object.values(redeemCodes).forEach((code) => {
            if (code.merchandise && code.status) { // Only count claimed ones
                const merch = code.merchandise;
                counts[merch] = (counts[merch] || 0) + 1;
            }
        });
        ...
        throw new Error("Error getting popular merchandise: ", error);
    }
};
