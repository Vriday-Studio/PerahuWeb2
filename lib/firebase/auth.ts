...
export const addLogUserLogin = async (userId: string | number, dataUser: any) => {
  ...
  const user = {
    id: dataUser.id || userId,
    Email: dataUser.Email,
    Nama: dataUser.Nama,
    lastLoggedIn: currentDate.toISOString(),
  }
...
export const getUserByEmail = async (email: string, specificKey?: string) => {
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      const user = Object.values(userData)[0] as any
...
export const signUp = async (data: any) => {
  try {
    ...
    const dataUser = {
      ...
      uid: res.user.uid,
      id: newUserId,
      avatar: data.gender === 'female' ? DEFAULT_AVATAR_FEMALE : DEFAULT_AVATAR_MALE,
      createdAt,
      updatedAt: createdAt,
    }
...
export const authWithProvider = async (selectedProvider: 'google', loginUser: any) => {
